...
//! data-source callback types live below. The `#[cfg(test)]` stubs here let the
//! standalone test binary link without the C++ shim.
//! [`crate::worker_sdk`] holds the `Event` primitive the workers pair with this
//! ABI.
...
extern "C" {
    ...
    // Flush the off-CPU writer before acking a flush/stop so its tail lands.
    pub fn sismo_ds_flush_offcpu(slot: u32);
...
mod stubs {
    ...
    #[no_mangle]
    pub unsafe extern "C" fn sismo_ds_flush_offcpu(_slot: u32) {}
    #[no_mangle]
...
#[cfg(test)]
pub use stubs::{
    sismo_ds_emit, sismo_ds_emit_offcpu, sismo_ds_flush_offcpu, sismo_ds_register,
    sismo_flush_done, sismo_stop_done,
};

...
//! Build the native C/C++ half of sismo:
//!
//! - The kernel-side BPF object (`csrc/sismo_bpf/sched.bpf.c`, Linux only) so
//!   the CPU collector can `include_bytes!` it via [`sismo_sys::BPF_OBJ`]. Needs
//!   clang with the bpf target.
//! - Behind the `perfetto-shim` feature: the Perfetto C++ SDK archive and the
//!   C/C++ shims (`csrc/*.cc`, `sample_target_sdk.c`), plus the link directives
//!   for the final binary. Enabled by the `sismo` binary crate; left off for
//!   plain `sismo-core` builds (e.g. `cargo test`) so they skip the expensive
//!   Perfetto compile.
//!
...
fn main() {
    ...
    // The BPF object is needed by sismo-core (Linux) regardless of the feature.
    if target_os == "linux" {
        build_bpf();
    }

    // The Perfetto shim compile + link only happens for the crate that builds
    // the final binary, which turns on the feature.
    if std::env::var_os("CARGO_FEATURE_PERFETTO_SHIM").is_some() {
        build_perfetto_shim(&target_os);
    }

    println!("cargo:rerun-if-changed=build.rs");
}

/// Compile `csrc/sismo_bpf/sched.bpf.c` to `sched.bpf.o` in OUT_DIR.
fn build_bpf() {
...
/// Build the Perfetto archive + C/C++ shims and emit the link surface for the
/// final binary.
fn build_perfetto_shim(target_os: &str) {
    // crates/sismo-sys -> repo root is two levels up.
...
/// Compile+link a tiny libc++-using program with `zig c++ -v`, scrape the
/// libc++/libc++abi/libunwind archive paths it links, and emit them as static
...
fn link_cxx_runtime(root: &PathBuf, out: &PathBuf) {
    ...
    for tok in log.split_whitespace() {
        if (tok.ends_with("/libc++.a")
            ...
            && !archives.contains(&tok.to_string())
        {
            archives.push(tok.to_string());
        }
    }
    assert!(
        archives.len() >= 3,
        "could not discover Zig C++ runtime archives from `zig c++ -v`; found {archives:?}\nstderr:\n{log}"
    );

    // Turn /path/to/libNAME.a into a (search-dir, NAME) static link. The names
    // carry '+' (c++, c++abi); rustc passes them through to the linker, which
    // resolves libNAME.a from the search dir.
    ...
    for archive in &archives {
        ...
        let name = path
            ...
            .unwrap_or_else(|| panic!("unexpected cxx runtime archive name: {archive}"))
            .to_string();
